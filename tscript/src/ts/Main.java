...
public class Main {
  ...
  private static void GenFunctionClass(Encode.ReturnValue a) {
    // get the class pool
    ...
    CtClass theClass = pool.makeClass(a.result);
    ...
    try {
      ...
      Message.fatal("cannot add a default constructor to the Func1 class: "
          + ex.getReason());
    }

    // now make a execute method
    String theMethodBody = a.code;

    CtMethod theMethod = null;
    try {
      theMethod = CtNewMethod.make(theMethodBody, theClass);
    } catch (CannotCompileException ex) {
      Message.fatal("cannot compile the generated Func1 method body: "
          + ex.getReason());
    }
    ...
    try {
      ...
      Message.fatal("cannot add the execute method into the Func1 class: "
          + ex.getReason());
    }
  }
  ...
  public static void main(String[] args) throws IOException {
    ...
    try {
      ...
      Location loc = new Location(errorToken.getTokenSource().getSourceName(),
          errorToken.getLine(), errorToken.getCharPositionInLine());
    ...
    for (Object item : root) {
      assert (item instanceof Statement) : "non-statement in statement list at root of AST";
      count += 1;
    }
    ...
    if (doExec) {
      List<Encode.ReturnValue> fL = genCode.getfL();
      for (Encode.ReturnValue e : fL) {
        GenFunctionClass(e);
      }
